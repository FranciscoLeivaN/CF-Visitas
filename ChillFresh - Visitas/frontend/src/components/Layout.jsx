    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-black text-white transition-all duration-300 ease-in-out overflow-y-auto`}>
        ...
        <nav className="mt-5">
          ...
          {/* Opción Mantenedor con Acordeón */}
          <div className="px-4 py-2">
            <button 
              ...
              className={`flex items-center justify-between w-full p-2 rounded-md transition-colors ${
                ['mantenedor', 'mantenedor-cultivos', 'mantenedor-inspectores', 'mantenedor-productores', 'mantenedor-usuarios'].includes(activeMenu) 
            ...
            >
              ...
              {sidebarOpen && <ChevronDownIcon className={`transition-transform ${mantenedorOpen ? 'transform rotate-180' : ''}`} />}
            </button>
            
            {/* Submenú del Mantenedor - Visible solo cuando mantenedorOpen es true */}
            {sidebarOpen && mantenedorOpen && (
              <div className="mt-2 ml-6 space-y-2">                {/* Opción Cultivos */}
                <button 
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white shadow-sm z-10">
          <div className="px-4 py-3 flex justify-between items-center">
            {/* Título de la sección activa */}            <div>
              <h1 className="text-2xl font-semibold text-black">
                {activeMenu === 'dashboard' && 'Dashboard'}
                {activeMenu === 'visitas' && 'Gestión de Visitas'}
                {activeMenu === 'reportes' && 'Reportes'}
                {activeMenu === 'mantenedor-cultivos' && 'Mantenedor - Cultivos'}
                {activeMenu === 'mantenedor-inspectores' && 'Mantenedor - Inspectores'}
                {activeMenu === 'mantenedor-productores' && 'Mantenedor - Productores'}
                {activeMenu === 'mantenedor-usuarios' && 'Mantenedor - Usuarios'}
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="hidden md:inline text-black">{user?.name || 'Usuario'}</span>
              </div>
            </div>
          </div>
        </header>
